...
class RRT():
    ...
    def _rectangles_overlap(self, r1, r2):
        ...
        def project(corners, axis):
            dots = [(np.dot(np.array(p), axis)) for p in corners]
            return min(dots), max(dots)

        axes = get_axes(r1) + get_axes(r2)
    ...
    def __init__(self, start, goal, obstacleList, randArea, alg, geom, dof=2, expandDis=0.05, goalSampleRate=5, maxIter=100):
        ...
        if self.geom == "rectangle":
            if len(self.start.state) == 2:
                self.start.state = [self.start.state[0], self.start.state[1], 0.0]
            if len(self.end.state) == 2:
                self.end.state = [self.end.state[0], self.end.state[1], 0.0]
            self.dof = 3
    ...
    def generatesample(self):
        ...
        if self.geom == "rectangle":
            if random.randint(0, 100) > self.goalSampleRate:
                ...
                rnd = Node([self.end.state[0], self.end.state[1], 0.0])
            return rnd
    ...
    def draw_graph(self, rnd=None):
        ...
        if self.goalfound:
            ...
            if self.geom == "rectangle":
                ...
                for px, py, th in path:
                    ...
                    world = (R @ corners.T).T + np.array([px, py])

                    poly = mpatches.Polygon(world, closed=True, fill=False, edgecolor='red')
        ...
        if self.geom == 'circle':
            radius = 1.0
            cs = mpatches.Circle((self.start.state[0], self.start.state[1]), radius, fill=False, linewidth=1.5, edgecolor="blue")
            plt.gca().add_patch(cs)
            
            cg = mpatches.Circle((self.end.state[0], self.end.state[1]), radius, fill=False, linewidth=1.5, edgecolor="red")
            ...
            R = np.array([
                [np.cos(sth), -np.sin(sth)],
                [np.sin(sth),  np.cos(sth)]])
            
            world = (R @ corners.T).T + np.array([sx, sy])
            
            start_poly = mpatches.Polygon(world, closed=True, fill=False, edgecolor='blue', linewidth=1.5)
            plt.gca().add_patch(start_poly)

            gx, gy, gth = self.end.state
            corners = np.array([
                [-L/2, -W/2],
                [ L/2, -W/2],
                [ L/2,  W/2],
            ...
            R = np.array([
                [np.cos(gth), -np.sin(gth)],
                [np.sin(gth),  np.cos(gth)]])
            
            world = (R @ corners.T).T + np.array([gx, gy])
            
            goal_poly = mpatches.Polygon(world, closed=True, fill=False, edgecolor='red', linewidth=1.5)
